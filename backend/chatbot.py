...
회의 전에는 아래 사항을 준비하면 좋아요.

### ✔ 회의계획(안) 작성
- 회의 목적, 참석자, 일정, 장소, 안건 등을 포함하여 **회의계획(안)** 작성  
- 필요한 경우 회의계획(안) 양식을 참고하여 작성  
- 회의비는 **1인당 5만원 한도**로 사용 가능하므로 예산 산정 시 유의  
- 회의실은 `그룹웨어 → 캘린더 → 자원관리`에서 예약 가능

작성 완료 후  
`그룹웨어 → 전자결재 → 내부결재`에서 상신하면 됩니다.
""",
    # 7️⃣ 회의 당일 준비사항 및 회의 후 해야 할 일
    """## 회의 당일 준비해야 할 사항

### ✔ 기본 준비
- 상신한 계획안이 결재 완료되었는지 확인  
- 참석자 명패 준비 (명패 양식 참고)  
- 서명부 출력 (서명부 양식 참고)

### ✔ 현장 준비
- 회의 전 다과 미리 구매 및 세팅 (영수증 반드시 보관)  
- 녹음기 미리 켜두기  
- 필요한 자료는 미리 출력해 책상에 배치  
- 참석자 도착 및 서명 안내 후 회의 진행
""",
    # 8️⃣ 회의 후 해야 할 일
    """##  회의 후 해야 할 일 (3가지)
...
### 2) 지출결의서 작성
- 법인카드 사용 시 **지출결의 필수**  
- 영수증 원본을 영수증첨부철에 붙여 첨부  
- 100만원 이상은 **본부장 전결**  
- 그룹웨어→전자결재→지출결의 에서 거래처 수에 따라 양식을 선택

### 3) 회의록 작성
- 참석하지 못한 직원들을 위한 공식 기록  
- 회의록 양식 참고하여 작성  

지출결의 때는 필수로 영수증을 제출해야하기 때문에,

영수증 첨부철에 원본영수증을 붙여서 붙임으로 첨부해야합니다.  (영수증첨부철 양식 던지기)

*주의: 지출결의 시, 지출금액이 100만원 이상일경우엔 본부장 전결로 진행해야 합니다
""",
...
작성 후 전자결재로 상신  
출장일자 또는 목적지가 정확하게 입력되었는지 반드시 재확인하세요.
""",
    # 10 출장 후(복명서‧지출결의) 수행해야 하는 일 답변 (A)
    """## 출장 후 해야 할 일

출장을 마쳤다면 아래 절차를 진행해야 해요.
...
- 출장 목적  
- 주요 추진내용  
- 일비, 식비, 숙박비, 교통비 등 비용 정산 내역 포함

### 2) 출장비 지출결의서 작성
- **국내출장:** 1주일 이내  
- **국외출장:** 2주일 이내  
- 운임, 숙박비 등 **세부 사용내역 증빙** 첨부 필수  
- 출장지 식사 시: 출장비 중 식대 25,000원에서 **1끼 8,000원 차감**  
  (※ 관외출장 시에만 식비 지급)

경로: `전자결재 → 지출결의서`  
- 계좌번호가 본인 계좌와 일치하는지 확인  
- 영수증 원본은 영수증 첨부철에 부착 후 결재 시 첨부 
- 출장비 정산 시, 여비와 관련된 사항은 여비지급 규칙을 참고! 
"""
...
FAQ_KEYWORDS = [
    ...
    ["출장신청", "여비정산"],
    ["전산장비", "PC", "프린터", "시설물", "고장"],
    ["회의 준비", "회의 전", "회의계획안", "회의 계획", "회의 어떻게"],
    ["회의 당일", "회의날", "회의 당일 준비"],
    ["회의 후","회의가 끝난 후", "결과보고", "회의 끝", "회의록", "지출결의"],
    ["출장 가기 전", "출장 전", "출장 처음", "출장신청서", "처음 가보는 출장인데 어떻게하면 좋을지"],
    ["출장 끝", "출장 다녀오고", "출장 후", "출장복명서", "출장 지출결의", "출장을 갔다와서"],
]
